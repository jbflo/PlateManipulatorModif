...
class PlateManipulator(SampleChanger):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.cmd_move_plate_to_location = None
    ...
    def init(self):
        ...
        self.plate_label = self.getProperty("plateLabel")
        self.crims_url = self.getProperty("crimsWsRoot")

        self.cmd_move_plate_to_location = self.get_command_object("startMovePlateToLocation")
        ...
        self.cmd_move_to_crystal_position = self.get_command_object("startMoveToCrystalPosition")
    ...
    def load(self, sample=None, wait=True):
        ...
        ret = self.load_sample(coords)
        self._set_loaded_sample(sample)
        return ret

    def change_location(self, sample, wait=True) :
        if ("row" in sample and "col" in sample and "pos_x" in sample):
            self.cmd_move_plate_to_location(ord(sample["row"]) - 65, sample["col"], sample["pos_x"] , sample["pos_y"] )
            self._set_loaded_sample(self.get_loaded_sample())
        else:
            return False
    ...
    def sync_with_crims(self):
        processing_plan = self._load_data(self.get_token())

        return processing_plan

    def move_to_crystal_position(self):
        """
         .
        """
        ...
        if self.processing_plan:
            for x in self.processing_plan.plate.xtal_list:
                if ( row == ord(x.row) - 65  and  col == x.column -1 and drop == x.shelf -1):
                    if self.cmd_move_to_crystal_position:
                        try:
                            ret = self.cmd_move_to_crystal_position(row, col, drop, x.get_image_as_byte_array(), x.offset_x, x.offset_y,
                            0.0, 0.0, True)
                        except:
                            ret = "Could not move to crystal position"
                    else:
                        ret = "command not found"
    
        else : ret = "No self.processing_plan found"
    ...
    def get_scan_limits(self, args):
        ...
        if self.cmd_get_omega_scan_limits:
            try:
                ...
                ret = "Could not get Omega Motor Dynamic Scan Limits"
        else:
            ret = "command not found"
