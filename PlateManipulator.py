...
class PlateManipulator(SampleChanger):
    ...
    def init(self):
        ...
        self.plate_barcode = self.getProperty("PlateBarode")
        self.harvester_key = self.getProperty("harvesterKey")
    ...
    def sync_with_crims(self):
        self.processing_plan = self._load_data()
    ...
    def _load_data(self):
        processing_plan = Crims.get_processing_plan(self.plate_barcode, self.crims_url, self.harvester_key)
        if processing_plan is None:
            msg = "No information about plate with barcode %s found in CRIMS" % self.plate_barcode
            logging.getLogger("user_level_log").error(msg)
        else:
            msg = "Information about plate with barcode %s found in CRIMS" % self.plate_barcode
            ...
            for x in self.processing_plan.plate.xtal_list:
                if (row == ord(x.row) - 65  and  col == x.column -1 and drop == x.shelf -1):
                    if self.cmd_move_to_crystal_position:
                        try:
                            ret = self.cmd_move_to_crystal_position(row, col, drop, x.image_url, x.offset_x, x.offset_y, 0.0, 0.0, True)
                        except Exception as ex:
                            raise Exception("Could not move to crystal position %s" %str(ex))
                    else:
                        raise Exception(" move_to_crystal_position command not found")
                else:
                    raise Exception(" No CRIMS Cristal found in this Position")
